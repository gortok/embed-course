...
adc_task (void * p_arg)
{
    ...
    for (;;)	
    {
      ...
      uint32_t result = (uint32_t)OSQPend(&g_adc_q, 0, OS_OPT_PEND_BLOCKING, &size, 0, &err);
      char p_str[12];
      sprintf(p_str, "sample is: %4u", (uint16_t)result);
      BSP_GraphLCD_String(7, p_str);
      if (result > 524) {
        OSFlagPost(&adc_flag_group, HIGH_ALARM, OS_OPT_POST_FLAG_SET, &err);   
      }
      else if (result > 500 && result < 524) {
        OSFlagPost(&adc_flag_group, MEDIUM_ALARM, OS_OPT_POST_FLAG_SET, &err);
      }
      else if (result > 0 && result < 500) {
        OSFlagPost(&adc_flag_group, LOW_ALARM, OS_OPT_POST_FLAG_SET, &err);
      }
      else {
        OSFlagPost(&adc_flag_group, NO_ALARM, OS_OPT_POST_FLAG_SET, &err);
      }
...
adc_isr (void)
{
    ...
    OSQPost(&g_adc_q, (void *)sample, 1, OS_OPT_POST_FIFO, &err);        
